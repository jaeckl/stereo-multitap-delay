...
template <typename SampleType> class DelayLine {
public:
  DelayLine()
      : DelayLine(100, {}) {}
  ...
  SampleType processSample(SampleType sample) {
    m_buffer.setSample(0, m_writeIndex, sample);
  ...
  int addReadingHead(int delayInSamples, float mixingAmplitude) {
    jassert(juce::isPositiveAndBelow(delayInSamples, m_buffer.getNumSamples()));
